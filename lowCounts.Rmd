...
If a few samples were a few (i.e. can count on fingers of one hand) chironomids short of the promised fifty, I would not object. In the Polish calibration set, nine of the forty-eight lakes (two of the reported fifty lakes have no chironomid data), are now reported to have fewer chironomids that promised, but how many fewer?

Since most samples should have at least one singleton, we can estimate the probably count sum from the reciprocal of the relative abundance of the least abundant taxa. 
...
Second, in addition to the reported nine Polish lakes with counts below fifty head capsules, a further `r as.eng(sum(lowCountRichness$minpc[lowCountRichness$country == "Poland"] > 2) - 9)` have a minimum relative abundance greater that 2% percent, indicative of a count lower than fifty heads.

What about Canadian portion of the calibration set? There are `r sum(lowCountRichness$country == "Canada")` Canadian lakes (not 72 as reported in LT15 - the data source, [Larocque (2008)](http://espace.inrs.ca/1020/1/R001032.pdf), reports 73 lakes). `r as.eng(sum(lowCountRichness$minpc[lowCountRichness$country == "Canada"] > 2), TRUE)` of these lakes appear to have a minimum count below fifty. 
...
This possible undercount in a few samples in Larocque (2008) in interesting but will not materially affect the results of that paper.

There are some additional curious aspects to the sums of the relative abundances for each lake in the calibration set. These can be below 100% if rare or unidentified taxa have been omitted. Values slightly above 100% are possible with rounding errors. But I struggle to understand how a sum of `r round(sum(as.data.frame(spp)["GIL",]), 1)`% is possible for Polish lake GIL. The sum of `r round(sum(as.data.frame(spp)["Lac H",]), 1)`% for Canadian Lac H is beyond comprehension.
