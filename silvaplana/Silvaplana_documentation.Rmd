...
```{r overlay_jopl_6a, fig.cap= "Figure 4. Archived reconstruction plotted on top of the published _JoPL_ figure 6a"}
```
...
Although assemblage data are only archived for the 540 yr _Holocene_ stratigraphy, stratigraphic diagrams are presented in all three papers and can be compared for the shared period. Below I compare the records for three species.
...
```{r Microtendipes, fig.cap = "Figure 5a. _Microtendipes_ percent. From left to right, _JoPL_, _Holocene_ archived, _Holocene_, _QSR_. The _JoPL_ stratigraphy is plotted against depth, so is non-linear in time.", cache = FALSE}
```

The stratigraphies seem to match for _Microtendipes_.


```{r Cricotopus, fig.cap = "Figure 5b. _Cricotopus_ percent", cache = FALSE}
```
...
```{r Procladius, fig.cap = "Figure 5c. _Procladius_ percent", cache = FALSE}
```

The records for _Procladius_ seem similar.

Across the three taxa examined, there are some differences.
...
Raw count data need to be archived for all the papers along with the calibration set. Explanations of any chronological adjustments made need to be given.

This document is reproducible with code archived on [github](https://github.com/richardjtelford/Zabinskie).
