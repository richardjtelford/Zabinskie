...
#short series
normclim <- dir(pattern = "\\.csv$", path = "data/luoto/", full.names = TRUE) %>% 
  grep(pattern = "luoto\\.csv", x = ., invert = TRUE, value = TRUE) %>% 
...
#long series
helsinki <- read_table("data/luoto/helsinki_t2974.dat", comment = "#", col_names = c("year", month.abb)) 
...
# % Year, Month, Temperature, Anomaly, Failed,   Breaks,   Temperature, Anomaly, Temperature, Anomaly
  helsinki_k <- read_table("data/luoto/13544-TAVG-Data.txt", skip = 71, col_names = c("year", "month", "raw_temperature", "raw_anomaly", "QC_failed", "continuity_breaks", "adj_temperature", "adj_anomaly","regional_temperature", "regional_anomaly")) %>% 
...
normclim %>% spread(key = station, value = Jul) %>% 
  ggplot(aes(x = Helsinki_K, y = Joutsa_Sav)) + 
  geom_point() + 
  geom_smooth()
normclim %>% spread(key = station, value = Jul) %$% 
