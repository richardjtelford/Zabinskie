...
Larocque-Tobler et al. [-@LarocqueTobler2012] use the calibration-in-time model from Larocque-Tobler et al. [-@LarocqueTobler2011b] to reconstruct July air temperature from a millennium-long chironomid stratigraphy from Seebergsee. In view of the very poor cross-validated performance of the calibration-in-time model (r^2^ = `r sbs_perf$crossval[2, "R2"]`), the reconstruction from Larocque-Tobler et al. [-@LarocqueTobler2012] cannot be expected to be skilful.
...
Various tools have been developed to identify data that are invalid due to errors or misconduct. Deviations from the expected distribution of digits from the Newcomb-Benford law has been used to detect issues with scientific and financial data [@Barabesi2017]. Carlisle [-@Carlisle2017] identified papers where the baseline differences in means for different treatments were surprisingly high or low given the variance of the data. Brown et al. [-@Brown2017] develop a simple test, GRIM, of whether means of integer data are consistent with the reported sample size.
