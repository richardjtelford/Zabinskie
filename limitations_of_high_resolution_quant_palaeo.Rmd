...
write - zabinskie ordination. long axes, low L1/L2, albeit significant, misreported % explained. interpretation.
...
Zhang et al. [-@Zhang2017] report that they correlate their July air temperature reconstruction with ###
mis-processed their instrumental time series. Rather than using a three year moving average as reported, they take the July air temperature of every third year and interpolate.  The significant correlation between the reconstruction and the interpolated triennial data can only be a Type 1 error, unless it is believed that chironomids are only sensitive to July temperature every third year and that they can predict the temperature up to two years ahead. With a three year moving average, the correlation is weak (r = `r zhang_smo_cor$estimate`) and not statistically significant (p = `r zhang_smo_cor$p.value`).
