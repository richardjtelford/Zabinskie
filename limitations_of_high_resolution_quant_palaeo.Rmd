...
Calibration-in-time models are routinely used in dendroclimatology [@Sheppard2010]. They have been used in palaeolimnology where calibration-in-space models are not appropriate because the model need tailoring to a specific site. They have been used to calibrate varve thickness against climate, especially for reconstructing summer temperature from proglacial lakes [@Leemann1994; @Thomas2009]. [#Ref] and [#ref] use a calibration-in-time approach to reconstruct ## from sediment reflectance data.
...
```{r table2}
read.table(header = TRUE, sep = "|", text = "Aspect|Favourable|Unfavourable|Reason
#------|---------|--------
Habitat|Plankic|Benthic|Simple taphonomy
...
Chronology|Varves|Other|More precise") %>% 
  knitr::kable(caption = "Aspects of reconstructions leading to favourable and unfavourable reconstruction")
```

The diatom-total phosphorus reconstructions [@Lotter1998; @Alefs1999] hold the most promise. The planktic diatoms are short-lived; nutrient availability has a strong control on diatom communities [@Tilman1981]; the taphonomy of sinking diatoms is relatively simple; and the slow changes in the target environmental variable mean that small chronological errors are inconsequential.
