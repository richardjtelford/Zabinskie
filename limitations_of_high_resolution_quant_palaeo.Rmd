...
```
```{r weather_climate_plot, fig.cap = "Fig. 3. Correlation between August and June (blue line), and August and summer temperatures (red line) averaged over different length periods in the central England temperature series [@Parker1992]"}
...
Doubts about the plausibility of annual and subdecadal resolution chironomid reconstructions do not imply that reconstructions on longer timescales [@Brooks2012] are flawed. At multi-decadal to centennial time scales, most of the issues discussed above are mitigated: single month and whole summer temperatures correlate well with each other as climate forcings rather than weather dominate; secondary variables such as dissolved organic carbon might correlate with climate change in time as they do in space and exert some influence on the assemblages; and lags due to taphonomic processes and phenology become largely irrelevant. It is not yet clear at what time scale the signal in the chironomid-inferred reconstruction can be expected to emerge from the noise.
...
The surprisingly poor reconstruction diagnostics are not the only problematic aspect of Larocque-Tobler et al. [-@Larocque-Tobler2015]. The instrumental temperature series used by Larocque-Tobler et al. [-@Larocque-Tobler2015] has an excellent correlation (r = `r i_post`) with a regional composite of long instrumental temperature series from 1939 CE onwards. Pre-1939 CE, the period in which chironomid samples were merged because of low abundances, the correlation deteriorates substantially (r = `r i_pre_mean`) if the mean temperature corresponding to each merged sample is used. Conversely, if the temperature from the first year corresponding to each merged sample is used, the correlation remains strong (r = `r i_pre_thinned`). It would appear that the temperature for the pre-1939 section of the record has been miscalculated. Despite this, the pre-1939 section of the reconstruction has a better correlation (r = `r pre_thinned$estimate`, p = `r pre_thinned$p.value %>% format.pval(eps = 0.001)`) with the incorrectly calculated temperature data than with the correct temperature data (r = `r pre_mean$estimate`, p = `r pre_mean$p.value`).
...
```{r count_error_plot, fig.cap="Figure 4. Standard deviation of WAPLS reconstuction based on samples with different count sums."}
count_error_plot
```
...
```{r Lac_AH}
```
The calibration set also has problems. Lac A and Lac H have identical assemblages `r stopifnot(A_equal_H)`, except that the percentages in Lac H have been multiplied by `r min_lac_H %>% format(digits = 3)` such that the sum of percent is `r rowSums_lac_AH[2]`%. 
